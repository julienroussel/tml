...
Vercel preview deployments (`tml-*-jaro-labs.vercel.app` — `e2e.yml` selects them by the `jaro-labs` team slug in the deployment URL) **cannot** validate server-side sync. Preview Vercel environments point `NEON_AUTH_BASE_URL` at a different Neon Auth instance than production; the production PowerSync Cloud instance trusts only production's JWKS + audience, so preview-issued JWTs are rejected when the sync stream is established (`PSYNC_S2105`). Since the `@powersync/web` 2.0 upgrade the default transport is HTTP rather than a WebSocket, so this surfaces on the streaming HTTP request rather than at a WebSocket handshake — the rejection and its consequences are unchanged. The sync stream never connects — and because PowerSync only calls `uploadData()` while the sync stream is connected, the upload queue never drains either.
