...
- Migration `0021_lockdown_sequence_function_acls.sql` (issue #253) extends the lockdown to **sequences** (no `USAGE`/`UPDATE` for `authenticated`) and **functions** (no `EXECUTE`). Today the schema uses UUID PKs and has zero `SECURITY DEFINER` functions, so the holes were dormant — but the new defaults guard against future SERIAL/IDENTITY columns and `SECURITY DEFINER` helpers, both of which would otherwise auto-grant privileges that bypass RLS and column-level GRANTs.
  ...
  - Any new `SECURITY DEFINER` function/procedure/aggregate in `public` MUST include explicit `REVOKE EXECUTE ON FUNCTION <name>(...) FROM PUBLIC, authenticated;` plus targeted `GRANT EXECUTE` only to the roles that need it. (`ALTER DEFAULT PRIVILEGES … ON FUNCTIONS` and `REVOKE … ON ALL FUNCTIONS` cover all three object classes per Postgres semantics.)
- Migration `0022_revoke_public_function_execute.sql` (issue #253 followup) closed an incomplete-lockdown hole left by 0021. 0021's `REVOKE EXECUTE … FROM authenticated` was a no-op because `authenticated` inherits EXECUTE from `PUBLIC` (never via direct grant) and `CREATE FUNCTION` auto-grants `EXECUTE` to `PUBLIC` regardless of default ACLs. 0022 adds `ALTER DEFAULT PRIVILEGES … REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC` (forward-looking) and `REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM PUBLIC` (sweep of the 3 existing invoker-rights helpers `set_updated_at`, `bump_updated_at_on_fk_null`, `show_db_tree`). Triggers continue to fire — Postgres checks trigger-function `EXECUTE` against the trigger definer (`neondb_owner`), not the firing role. **Caveat**: `ALTER DEFAULT PRIVILEGES FOR ROLE neondb_owner` is grantor-scoped — out-of-band function creation by a different role (e.g., a Neon-console `postgres` session) re-introduces the auto-grant. Discipline: only Drizzle migrations (which run as `neondb_owner`) should create public-schema functions.
